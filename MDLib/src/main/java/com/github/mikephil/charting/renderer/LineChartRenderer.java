...
public class LineChartRenderer extends LineRadarRenderer {
    public boolean showZeroPointer = true;
...
//        mDrawBitmap.get().eraseColor(Color.GREEN);
...
//            Entry prev = dataSet.getEntryForIndex(mXBounds.min);
            Entry prev = dataSet.getEntryForIndex(pos);
            Entry cur = prev;
            if (!showZeroPointer){
//            if (true){
                if (prev.getY()>=0){
                    for (int i = 0; i <= mXBounds.range + mXBounds.min; i++) {
                        prev = dataSet.getEntryForIndex(i);
//                        if (prev.getY()>0){
                        if (prev.getY()>=0){
                            cur = prev;
                            pos = i;
...
//            for (int j = mXBounds.min + 1; j <= mXBounds.range + mXBounds.min; j++) {
            for (int j = pos + 1; j <= mXBounds.range + mXBounds.min; j++) {
                prev = cur;
...
//        mRenderPaint.setColor(Color.GREEN);
    ...
    protected void drawLinear(Canvas c, ILineDataSet dataSet) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            Entry e1, e2;
            int pos = mXBounds.min;

            e2 = dataSet.getEntryForIndex(pos);
            if (!showZeroPointer){
                for (int i = mXBounds.min; i <= mXBounds.range + mXBounds.min; i++) {
                    e2 = dataSet.getEntryForIndex(i);
                    if (e2.getY()>0){
                        pos = i;
                        break;
                    }else {
            ...
            if (e2 != null) {
                ...
                int j = 0;
                for (int x = pos; x <= mXBounds.range + mXBounds.min; x++) {
                    e1 = e2;
...
//            mCirclePaintInner.setColor(Color.GREEN);
...
//                if (!showZeroPointer){
    ...
    public void setShowZeroPointer(boolean showZeroPointer) {
        this.showZeroPointer = showZeroPointer;
    }
}
