   ...
   // private String mNoDataText = "No chart data available.";
    ...
    public void setData(T data) {
       ...
       setupDefaultFormatter(data.getYMin(), data.getYMax());
        //setupDefaultFormatter(data.getYMin(),(float)10.0);
    ...
    protected void setupDefaultFormatter(float min, float max) {
       ...
       // mDefaultValueFormatter.setup(digits);
        mDefaultValueFormatter.setup(1);
    }
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            if (getHighlighter() instanceof ChartHighlighter){
                List<Entry> entries = new ArrayList<>();
                ChartHighlighter chartHighlighter = (ChartHighlighter) getHighlighter();
                List<Highlight> list = chartHighlighter.getHighlightBuffer();
                for (Highlight h : list){
                    Entry entry = mData.getEntryForHighlight(h);
                    entries.add(entry);
                }
                // callbacks to update the content
                mMarker.refreshContent(e, highlight, entries);
            }else {
                // callbacks to update the content
                mMarker.refreshContent(e, highlight, null);
            }
