...
public class WheelView extends View {
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    label = attribute.getString(R.styleable.WheelView_label_String);
    labelTextSize = attribute.getDimension(R.styleable.WheelView_label_TextSize,labelTextSize);
    labelTextColor = attribute.getColor(R.styleable.WheelView_label_TextColor,labelTextColor);
  ...
  private void drawLable(Canvas canvas) {
    if (!TextUtils.isEmpty(label)){
      ...
      for (int i = 0; i < dataList.size(); i++) {
        bounds = new Rect();
        textPaint.getTextBounds(dataList.get(i), 0, dataList.get(i).length(), bounds);
        int textWidth = bounds.right - bounds.left;
        textWidths.add(textWidth);
        textHeights.add(bounds.bottom-bounds.top);
      }
      if (maxTextWidth==0){
//        Integer max = Collections.max(textWidths);
        Integer max = ArrayListMax(textWidths);
        textX = (controlWidth / 2f + max/ 2f);
      }
      textX = (controlWidth / 2f + maxTextWidth/ 2f);
//      Integer maxHeight = Collections.max(textHeights);
      Integer maxHeight = ArrayListMax(textHeights);
      textY =  getHeight()/2+maxHeight/2+textHeight/3;
  ...
  public void setDefault(int index) {
    ...
    if (onSelectListener != null){
      if (!TextUtils.isEmpty(dataList.get(index))){
//        onSelectListener.endSelect(index,dataList.get(index));
      }
    }
  }
...
//    private Rect textRect;
    ...
    public void drawSelf(Canvas canvas, int containerWidth) {
      ...
      // 判断是否可被选择
      if (couldSelected()) {
//        selectedColor = Color.parseColor("#2494f3");
      ...
      canvas.drawText(itemText,
              ((float) x + controlWidth / 2f - (float) textRect.width() / 2f),
      ...
      if (couldSelected()){
        if (!isScrolling()){
          ...
          for (int i = 0; i < dataList.size(); i++) {
            bounds = new Rect();
            textPaint.getTextBounds(itemText, 0, itemText.length(), bounds);
            int textWidth = bounds.right - bounds.left;
            textWidths.add(textWidth);
          }
...
//          Integer max = Collections.max(textWidths);
    ...
    public synchronized boolean couldSelected() {
      ...
      if (y + move <= itemNumber / 2 * unitHeight - (unitHeight/2)
          || y + move >= itemNumber / 2 * unitHeight + unitHeight) {
        isSelect = false;
      }
//      isSelect = selected();
  ...
  public int ArrayListMax(List<Integer> sampleList)
  {
    try
    {
      int maxDevation = 0;
      ...
      if (totalCount >= 1)
      {
        int max = sampleList.get(0);
        for (int i = 0; i < totalCount; i++)
        {
          int temp = sampleList.get(i);
          if (temp > max)
          {
            max = temp;
          }
