...
public abstract class WeekView extends BaseView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (hasScheme) {
                ...
                if (isSelected) {
                    isDrawSelected = onDrawSelected(canvas, calendar, x, true,mDelegate.getSelectedThemeColor());
                }
                ...
                if (isSelected) {
                    onDrawSelected(canvas, calendar, x, false,mDelegate.getSelectedThemeColor());
                }
            }
   ...
   /* void updateWeekDateColor() {
        if (mItems == null)
            return;
        if(mDelegate.mPageScrollCalendar==null){
            mDelegate.mPageScrollCalendar = mDelegate.mSelectedCalendar;
        }
        for (Calendar a : mItems) {//添加操作
            if(a.isSameMonth(mDelegate.mPageScrollCalendar)){
                a.setCurrentMonth(true);
            }else{
                a.setCurrentMonth(false);
            }
        }
//        invalidate();
    }*/
     ...
     */
    protected abstract boolean onDrawSelected(Canvas canvas, Calendar calendar, int x, boolean hasScheme, int selectShemeColor);
