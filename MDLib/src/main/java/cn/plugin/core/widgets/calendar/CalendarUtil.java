...
public final class CalendarUtil {
    ...
    static List<Calendar> initCalendarForWeekView(Calendar calendar, CalendarViewDelegate mDelegate, int weekStart) {
        ...
        int dayCount = CalendarUtil.getMonthDaysCount(calendar.getYear(), calendar.getMonth());//获取某个月有多少天
        int weekEndDiff = getWeekViewEndDiff(calendar.getYear(), calendar.getMonth(), calendar.getDay(), weekStart);
        List<Calendar> mItems = new ArrayList<>();
        ...
        if (calendar.getDay() - week <= 0) {//如果某月某天-星期<0，则说明前面需要上个月的补数
            ...
            if (calendar.getMonth() == 1) {//取上一年的12月份
                ...
                preMonthDaysCount = CalendarUtil.getMonthDaysCount(calendar.getYear(), calendar.getMonth() - 1);
...
//            }
//            if(calendarDate.isSameMonth(mDelegate.mPageScrollCalendar)){
//                calendarDate.setCurrentMonth(true);
//            }
            calendarDate.setWeekend(CalendarUtil.isWeekend(calendarDate));
            calendarDate.setWeek(CalendarUtil.getWeekFormCalendar(calendarDate));
