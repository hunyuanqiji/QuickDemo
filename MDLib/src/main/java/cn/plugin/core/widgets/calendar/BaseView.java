...
public abstract class BaseView extends View implements View.OnClickListener, View.OnLongClickListener {
     ...
     * 当前月份今日之前的本月日期的笔
     */
    protected Paint mBeforeCurMonthTextPaint = new Paint();

    /**
     * 当前月份今日之后的本月日期的笔
     */
    protected Paint mAfterCurMonthTextPaint = new Paint();
    /**
     * 其它月份日期颜色
     */
    ...
    private void initPaint(Context context) {
        ...
        mCurMonthTextPaint.setTextSize(CalendarUtil.dipToPx(context, 25));

        mBeforeCurMonthTextPaint.setAntiAlias(true);
        mBeforeCurMonthTextPaint.setTextAlign(Paint.Align.CENTER);
        mBeforeCurMonthTextPaint.setColor(0xFFe1e1e1);
        mBeforeCurMonthTextPaint.setFakeBoldText(true);
        mBeforeCurMonthTextPaint.setTextSize(CalendarUtil.dipToPx(context, 18));
        ...
        mAfterCurMonthTextPaint.setAntiAlias(true);
        mAfterCurMonthTextPaint.setTextAlign(Paint.Align.CENTER);
        mAfterCurMonthTextPaint.setColor(0xFF111111);
        mAfterCurMonthTextPaint.setFakeBoldText(true);
        mAfterCurMonthTextPaint.setTextSize(CalendarUtil.dipToPx(context, 18));
        ...
        mSelectedLunarTextPaint.setColor(Color.WHITE);
        ...
        mSchemeTextPaint.setColor(Color.RED);
        //     mSchemeTextPaint.setColor(0xFFe1e1e1);
    ...
    void setup(CalendarViewDelegate delegate) {
        ...
        this.mCurMonthTextPaint.setColor(delegate.getCurrentMonthTextColor());//设置当前月份的字体的颜色，不是本月，当月是指对应上方条目显示的月份。
        ...
        this.mSchemeTextPaint.setColor(delegate.getSchemeTextColor());//满字日历字体的颜色
     ...
     * @param map
     */
    public void setZeroReservation(Map<String, Calendar> map) {
        this.map = map;
    }
     ...
     * 设置开放预约量为0
     */
    public Map<String, Calendar> getZeroReservation() {
        return map;
    }
     ...
     * 开放预约量为0
     */
    final void addZeroReservation() {
        if (mDelegate.zeroSchemeDatesMap == null || mDelegate.zeroSchemeDatesMap.size() == 0) {
            return;
        }
        for (Calendar a : zeroItems) {
            if (mDelegate.zeroSchemeDatesMap.containsKey(a.toString())) {
                Calendar d = mDelegate.zeroSchemeDatesMap.get(a.toString());
                a.setScheme(TextUtils.isEmpty(d.getScheme()) ? mDelegate.getSchemeText() : d.getScheme());
                a.setSchemeColor(d.getSchemeColor());
                a.setSchemes(d.getSchemes());
            } else {
    ...
    //设置字体是否加粗
    protected void setFakeBoldText(boolean effectiveDate) {
        mCurMonthTextPaint.setFakeBoldText(true);
    }
